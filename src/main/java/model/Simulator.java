...
public class Simulator {
	...
	private PriorityQueue<Patient> waitingRoom; //contiene solo i pazienti in lista d'attesa
	private int freeStudios; //numero studi medici liberi
	private Patient.ColorCode lastColor;
	...
	public void init() { //inizializza il simulatore
		//inizializza coda eventi e modello del mondo
		this.queue= new PriorityQueue<>();
		this.waitingRoom= new PriorityQueue<>();
		this.pazienti= new ArrayList<>();
		this.freeStudios=this.totStudios;
		this.lastColor=ColorCode.RED;
	...
	private Patient.ColorCode prossimoColore(){
		if(lastColor.equals(ColorCode.WHITE))
			lastColor= ColorCode.YELLOW;
		else if(lastColor.equals(ColorCode.YELLOW))
			lastColor= ColorCode.RED;
		else
			lastColor = ColorCode.WHITE;
		return lastColor;
	}
	
	public void run() { //esegue la simulazione
	...
	private void processEvent(Event e) {
		...
		case FREE_STUDIO: 
			if (this.freeStudios==0)
				return; //in questo caso equivalente al break perche non ho nulla dopo lo switch
			//Quale paziente fare entrare?
		...
		case TIMEOUT:
			...
			case WHITE:
				...
				break;
			case YELLOW:
				this.waitingRoom.remove(p); //perche gli cambio colore
